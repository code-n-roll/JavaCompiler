...
class Scanner {
    ...
    public TokenInfo getNextToken() {
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                if (ch == '/') {
                    ...
                    nextCh();
                    return new TokenInfo(DIV_ASSIGN, line, column);
                } else {
                    return new TokenInfo(DIV, line, column);
                }
            } else {
        ...
        case '(':
            nextCh();
            return new TokenInfo(LPAREN, line, column);
        case ')':
            nextCh();
            return new TokenInfo(RPAREN, line, column);
        case '{':
            nextCh();
            return new TokenInfo(LCURLY, line, column);
        case '}':
            nextCh();
            return new TokenInfo(RCURLY, line, column);
        case '[':
            nextCh();
            return new TokenInfo(LBRACK, line, column);
        case ']':
            nextCh();
            return new TokenInfo(RBRACK, line, column);
        case ';':
            nextCh();
            return new TokenInfo(SEMI, line, column);
        case ',':
            nextCh();
            return new TokenInfo(COMMA, line, column);
        case '=':
            nextCh();
            if (ch == '=') {
                nextCh();
                return new TokenInfo(EQUAL, line, column);
            } else {
                return new TokenInfo(ASSIGN, line, column);
            }
        case '!':
            nextCh();
            if (ch == '='){
                nextCh();
                return new TokenInfo(NOT_EQUAL, line, column);
            } else {
                return new TokenInfo(LNOT, line, column);
            }
        case '*':
            nextCh();
            if (ch == '='){
                nextCh();
                return new TokenInfo(STAR_ASSIGN, line, column);
            } else {
                return new TokenInfo(STAR, line, column);
            }
        case '%':
            nextCh();
            if (ch == '='){
                nextCh();
                return new TokenInfo(MOD_ASSIGN, line, column);
            } else {
                return new TokenInfo(MOD, line, column);
            }
        case '+':
            nextCh();
            if (ch == '=') {
                nextCh();
                return new TokenInfo(PLUS_ASSIGN, line, column);
            } else if (ch == '+') {
                nextCh();
                return new TokenInfo(INC, line, column);
            } else {
                return new TokenInfo(PLUS, line, column);
            }
        case '-':
            nextCh();
            if (ch == '='){
                nextCh();
                return new TokenInfo(MINUS_ASSIGN, line, column);
            } else if (ch == '-') {
                nextCh();
                return new TokenInfo(DEC, line, column);
            } else {
                return new TokenInfo(MINUS, line, column);
            }
        case '&':
            nextCh();
            if (ch == '&') {
                nextCh();
                return new TokenInfo(LAND, line, column);
            } else {
        ...
        case '|':
            ...
            if (ch == '|'){
                nextCh();
                return new TokenInfo(LOR, line, column);
            } else {
                reportScannerError("Operator | is not supported in j--.");
                return getNextToken();
            }
        case '>':
            ...
            if (ch == '='){
                nextCh();
                return new TokenInfo(GE, line, column);
            } else {
                return new TokenInfo(GT, line, column);
            }
        case '<':
            nextCh();
            if (ch == '=') {
                nextCh();
                return new TokenInfo(LE, line, column);
            } else {
                return new TokenInfo(LT, line, column);
//                reportScannerError("Operator < is not supported in j--.");
//                return getNextToken();
            }
        case '\'':
            ...
            if (ch == '\'') {
                buffer.append('\'');
                nextCh();
                return new TokenInfo(CHAR_LITERAL, buffer.toString(), line, column);
            } else {
        ...
        case '"':
            ...
            return new TokenInfo(STRING_LITERAL, buffer.toString(), line, column);
        case '.':
            nextCh();
            return new TokenInfo(DOT, line, column);
        case EOFCH:
        ...
        case '9':
            ...
            if (ch == '.'){
                ...
                return new TokenInfo(DOUBLE_LITERAL, buffer.toString(), line, column);
            }
            return new TokenInfo(INT_LITERAL, buffer.toString(), line, column);
        default:
            if (isIdentifierStart(ch)) {
                ...
                if (reserved.containsKey(identifier)) {
                    return new TokenInfo(reserved.get(identifier), line, column);
                } else {
                    return new TokenInfo(IDENTIFIER, identifier, line, column);
                }
            } else {
    ...
    /**
     * Report a lexical error and record the fact that an error has occured.
...
class CharReader {
    ...
    /** The variable for record column numbers. */
    ...
    /**
     * The current column number in the source file, starting at 1.
     *
     * @return the current column number.
     */

    public int column() {
        // ColumnNumber counts columns from 0.
